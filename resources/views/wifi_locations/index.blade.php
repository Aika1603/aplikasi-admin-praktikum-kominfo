                        ...
                        <tbody>
                            @foreach ($datatable as $key => $row)
                                <tr>
                                    <td>{{ $no++ }}</td>
                                    <td>{{ $row->name }}</td>
                                    <td>{{ $row->kecamatan_name }}<br/>{{ $row->desa_name }}</td>
                                    <td><?= $row->is_active == '0' ? '<span class="badge bg-danger">Non Active</span>' : '<span class="badge bg-success">Active</span>' ;?></td>
                                    <td class=''>
                                        <div class="list-icons">
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right">
                                                    @can($permission.'-detail')
                                                        <a href="{{ route($route.'.detail', $row->id) }}" class="dropdown-item"><i class="icon-list2"></i> Detail</a>
                                                    @endcan
                                                    @can($permission.'-edit')
